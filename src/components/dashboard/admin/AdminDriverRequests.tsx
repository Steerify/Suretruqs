    ...
    <Card className="p-5 border border-slate-100 shadow-sm mb-4">
        <div className="flex justify-between items-start mb-4">
            <div className="flex items-center gap-3">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black ${shipment.status === ShipmentStatus.PENDING ? 'bg-orange-50 text-brand-orange' : 'bg-blue-50 text-brand-primary'}`}>
                    <Package size={20} />
                </div>
                <div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-slate-50">
            <span className="text-sm font-black text-slate-900">₦{shipment.price.toLocaleString()}</span>
          ...
          <table className="w-full text-left text-sm">
            <thead>
                <tr className="bg-slate-50 text-slate-500 font-black uppercase tracking-widest text-[10px] border-b border-slate-100">
                    ...
                    <th className="px-8 py-5">Cargo & Price</th>
                    ...
                    displayedShipments.map(shipment => (
                        <tr key={shipment.id} className="hover:bg-slate-50/50 transition-colors group">
                            <td className="px-8 py-5">
                                <div className="flex items-center gap-3">
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black ${shipment.status === ShipmentStatus.PENDING ? 'bg-orange-50 text-brand-orange' : 'bg-blue-50 text-brand-primary'}`}>
                                        <Package size={18} />
                                    </div>
                                    <div>
                            ...
                            <td className="px-8 py-5">
                                <div>
                                    <p className="font-black text-slate-900">₦{shipment.price.toLocaleString()}</p>
                                    <p className="text-[10px] text-slate-500 font-medium capitalize">{shipment.cargoType} • {shipment.weight}kg</p>
                                </div>
                            </td>
                            <td className="px-8 py-5">
                                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border ${
                                    ...
                                    {shipment.status === ShipmentStatus.PENDING && <Clock size={10} className="animate-pulse"/>}
