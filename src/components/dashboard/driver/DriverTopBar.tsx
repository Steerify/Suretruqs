...
import { Truck, Home, Wallet, Clock, User as UserIcon, Power, Headphones, Bell, ChevronDown, LogOut } from 'lucide-react';
...
export const DriverTopBar = ({ user, view, setView, isOnline, setIsOnline, showNotifications, setShowNotifications, onLogout }: DriverTopBarProps) => {
    return (
      <div className="bg-white/90 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50 transition-all duration-300">
         <div className="w-full max-w-[1920px] mx-auto flex justify-between items-center px-6 h-20">
             ...
             <div className="flex items-center gap-3 lg:gap-5">
                 ...
                 >
                    <Power size={14} className={isOnline ? 'text-green-600' : 'text-slate-400'} />
                    {isOnline ? 'Available' : 'Offline'}
                    <div className={`w-2 h-2 rounded-full ml-1 ${isOnline ? 'bg-green-500 animate-pulse' : 'bg-slate-400'}`}></div>
                 </button>
