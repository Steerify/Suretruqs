...
export const DriverHistoryView = ({ jobHistory, setSelectedHistoryItem }: DriverHistoryViewProps) => (
     <div className="p-4 md:p-8 pb-24 fade-up w-full max-w-[1920px] mx-auto">
        ...
        <div className="space-y-4">
           ...
           {jobHistory.map(job => (
              <div key={job.id} className="bg-white rounded-2xl p-6 flex flex-col md:flex-row justify-between items-start md:items-center group border border-slate-200 shadow-sm hover:shadow-md transition-all gap-6">
                 <div className="flex items-start space-x-5 w-full md:w-auto">
                    <div className={`p-3 rounded-2xl shrink-0 ${job.status === 'DELIVERED' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
                    ...
                    <div className="flex-1">
                       ...
                       <div className="flex items-center gap-3 text-sm font-bold text-slate-900">
                          <span>{job.pickup.address.split(',')[0]}</span>
                          <span className="text-slate-300"><ArrowRight size={16}/></span>
                          <span>{job.dropoff.address.split(',')[0]}</span>
                       </div>
                    </div>
                 </div>
                 <div className="flex items-center justify-between w-full md:w-auto gap-8 pl-16 md:pl-0">
                    <div className="text-right">
                        ...
                        <p className="font-bold text-slate-900 text-lg">₦{job.price.toLocaleString()}</p>
                    </div>
                    <Button onClick={() => setSelectedHistoryItem(job)} variant="secondary" size="sm" className="rounded-xl px-4">Details</Button>
                 </div>
              </div>
           ))}
           {jobHistory.length === 0 && (
