...
export const DriverChatModal = ({ driverName, messages, onClose, onSendMessage, isTyping }: { driverName: string, messages: any[], onClose: () => void, onSendMessage: (text: string) => void, isTyping: boolean }) => {
          ...
          <div className="bg-slate-50 w-full md:max-w-md h-[85vh] md:h-[600px] md:rounded-3xl rounded-t-3xl shadow-2xl relative z-10 flex flex-col overflow-hidden animate-[slideUp_0.3s_ease-out]">
              ...
              <div className="bg-white border-b border-slate-200 p-4 flex items-center justify-between shadow-sm z-20">
                  <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center font-bold text-brand-primary border border-blue-200">
                          {driverName.charAt(0)}
                      </div>
                      <div>
                          <h3 className="font-bold text-slate-900 text-sm">{driverName}</h3>
